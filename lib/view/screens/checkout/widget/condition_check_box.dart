...
class CheckoutCondition extends StatelessWidget {
  ...
  const CheckoutCondition({Key? key, required this.orderController}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    bool activeRefund = Get.find<SplashController>().configModel!.refundPolicyStatus == 1;
    print('----------------------> ${Get.find<SplashController>().configModel!.refundPolicyStatus} / $activeRefund');
    return Row(children: [
      // SizedBox(
      //   width: 24.0,
      //   height: 24.0,
